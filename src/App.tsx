    ...
    <section style={{padding:'96px 24px', background:CREAM}}>
      <div style={{maxWidth:1100, margin:'0 auto', display:'flex', flexDirection:'column', gap:80}}>
        ...
        <motion.div initial={{opacity:0,y:40}} whileInView={{opacity:1,y:0}} viewport={{once:true}}
          ...
          {/* Scrolling tags - 2 rows moving opposite directions */}
          <div style={{background:'#fce4ec', borderRadius:20, overflow:'hidden', height:280, display:'flex', flexDirection:'column', justifyContent:'center', gap:12}}>
            {[tags.slice(0,9), tags.slice(9)].map((row, rowIdx) => (
                ...
                <motion.div
                  ...
                  {[...row,...row].map((t,i)=>(
                    <div key={i} style={{
                      ...
                      <div style={{width:20,height:20,borderRadius:'50%',background:t.good?'#1c3410':'#ef4444',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                        {t.good ? <Check size={11} color="white"/> : <X size={11} color="white"/>}
                      </div>
                      {t.label}
                    </div>
                  ))}
